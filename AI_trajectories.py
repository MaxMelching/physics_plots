...
def upper(T: float, t_wait: float = 0, z0: float = 0, v0: float = 0,
          k_eff: float = 0, g: float = 9.81, num: int = 100) -> tuple:
    times = np.linspace(0, 2 * t_wait + 2 * T, num)
    results = np.zeros(times.size)

    # Create masks to split times between pulses
    mask1 = times <= t_wait  # Before first Pi/2
    mask2 = (times > t_wait) & (times <= t_wait + T)  # Pi/2 - Pi
    mask3 = (times > t_wait + T) & (times <= t_wait + 2 * T) # Pi - Pi/2
    mask4 = times > t_wait + 2 * T  # After second Pi/2
...
def lower(T: float, t_wait: float = 0, z0: float = 0, v0: float = 0,
          k_eff: float = 0, g: float = 9.81, num: int = 100) -> tuple:
    times = np.linspace(0, 2 * t_wait + 2 * T, num)
    results = np.zeros(times.size)
...
if __name__ == '__main__':
    ...
    t_wait = pulse_time / 4  # Waiting time before, after pulses
    ...
    times, traject = upper(pulse_time, t_wait, v0=v_init, k_eff=v_added)
    plt.plot(times, traject)

    times, traject = lower(pulse_time, t_wait, v0=v_init, k_eff=v_added)
    plt.plot(times, traject)

    # Plot dashed lines indicating laser pulses
    plt.plot(2 * [t_wait], [0, traject[-1]], 'r--')
    plt.plot(2 * [t_wait + pulse_time], [0, traject[-1]], 'r--')
    plt.plot(2 * [t_wait + 2 * pulse_time], [0, traject[-1]], 'r--')
